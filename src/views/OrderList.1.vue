<template>
	<div class="px-4 pt-2">
		<v-card  outlined  id="vRow">
			<!-- <div class=" px-5 py-3 my-0 green white--text d-flex flex-column justify-start align-start">
				<div style="font-size: 18px;" v-once>
					全部记录
				</div>
				<div>
					<v-btn color="white" small outlined class="ma-1">
					...
					<v-btn color="white" small outlined class="ma-1" @click="search" >查询</v-btn>
					
				</div>
			</div> -->
			<div class=" px-5 py-2 d-flex flex-column justify-start align-start">
				<Badge color="green" :class-name="'title'" text="全部记录"></Badge>
			</div>
			<v-divider></v-divider>

			<div>
				<v-btn color="green" small outlined class="ma-1">
					<v-icon left>mdi-weather-sunny</v-icon>
					<date-pickers :value="startTime" @changeTime="startTimeChange"></date-pickers>
				</v-btn>
				<v-btn color="green" small outlined class="ma-1">
				...
				<v-btn color="green" small outlined class="ma-1" @click="search" >查询</v-btn>
			...
			<component :is="transition !== 'None' ? `v-${transition}` : 'div'"  hide-on-leave>
				...
				<div v-else>
					<v-list-item three-line class="xy-tableItem grey" v-if="items.length<1">
