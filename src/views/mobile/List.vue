	...
	<div class="pa-2">
		...
		<div class="d-flex flex-column mt-1" v-resize="draw" id="vRow">
			...
			<div class="d-flex align-center">
				<v-select outlined v-model="tabIndex"
				@change="changeTab" dense
				hide-details
				color="green" :items="typeArr" item-text="value"
				item-color="green"
				 item-value="id" label="计价货币" single-line></v-select>
				<v-btn color="green" dark depressed class="my-1 ml-1" @click="search" >查询</v-btn>
			</div>
			...
			<component :is="transition !== 'None' ? `v-${transition}` : 'div'"  hide-on-leave>
				...
				<div v-else>
					...
					<component v-else :is="componentArr[tabIndex]" ref="componentType" 
					:items="items"
					 @hook:mounted="doSomething"></component>
					<div class="text-center my-4" v-if="items.length>0">
						<v-pagination color="green" v-model="curPage.page" :length="curPage.size" v-on:input="inputShow" 
